...
const Blog = () => {
  return (
    <div className="px-6 py-10 bg-gray-100 dark:bg-[#1f1f1f]">
      ...
      <section className="w-[70%] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
        {posts.map((post) => (
          <div key={post.id} className="bg-white dark:bg-[#2b2b2b] p-6 rounded-lg shadow-md dark:shadow-lg">
            <img src={post.image} alt={post.title} className="w-full h-48 object-cover rounded-md" />
            <h2 className="mt-6 text-2xl font-semibold text-black dark:text-white">{post.title}</h2>
            <p className="mt-4 text-gray-600 dark:text-gray-300">{post.excerpt}</p>
            <Link to={`/blog/${post.id}`} className="mt-4 inline-block text-blue-600 dark:text-blue-400 hover:underline">
              Read More
            </Link>
          </div>
        ))}
